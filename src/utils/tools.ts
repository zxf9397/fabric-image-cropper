...
export function getZeroCoords(width: number, height: number) {
  const midW = width / 2,
    midH = height / 2;

  return {
    tl: new Point({ x: 0, y: 0 }),
    mt: new Point({ x: midW, y: 0 }),
    tr: new Point({ x: width, y: 0 }),
    mr: new Point({ x: width, y: midH }),
    br: new Point({ x: width, y: height }),
    mb: new Point({ x: midW, y: height }),
    bl: new Point({ x: 0, y: height }),
    ml: new Point({ x: 0, y: midH }),
  };
}
